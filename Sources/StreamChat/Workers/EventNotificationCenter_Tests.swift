...
final class EventNotificationCenter_Tests: XCTestCase {
    ...
    func test_eventsAreProcessed_fromWithinTheWriteClosure() {
        ...
        center.add(middleware: EventMiddlewareMock(closure: { event, session, completion in
            usedSession = session
    ...
    func test_eventsAreBatched() {
        ...
        let center = EventNotificationCenter(middlewares: [EventMiddlewareMock()], database: database)
